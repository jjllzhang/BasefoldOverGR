...
Bytes Sha256(const Byte *data, std::size_t len) {
  ...
  Bytes padded;
  padded.reserve(len + 1 + 64);
  padded.insert(padded.end(), data, data + len);
  padded.push_back(static_cast<Byte>(0x80));
  while ((padded.size() % 64) != 56)
    padded.push_back(static_cast<Byte>(0x00));

  const std::uint64_t bit_len = static_cast<std::uint64_t>(len) * 8;
  for (int i = 7; i >= 0; --i) {
    padded.push_back(static_cast<Byte>((bit_len >> (8 * i)) & 0xff));
  }

  std::array<std::uint32_t, 64> w;
  for (std::size_t chunk = 0; chunk < padded.size(); chunk += 64) {
    for (int i = 0; i < 16; ++i) {
      const std::size_t off = chunk + static_cast<std::size_t>(4 * i);
      w[static_cast<std::size_t>(i)] =
          (static_cast<std::uint32_t>(padded[off + 0]) << 24) |
          (static_cast<std::uint32_t>(padded[off + 1]) << 16) |
          (static_cast<std::uint32_t>(padded[off + 2]) << 8) |
          (static_cast<std::uint32_t>(padded[off + 3]) << 0);
    }

    for (int i = 16; i < 64; ++i) {
      const std::uint32_t s0 = RotR32(w[static_cast<std::size_t>(i - 15)], 7) ^
                               RotR32(w[static_cast<std::size_t>(i - 15)], 18) ^
                               (w[static_cast<std::size_t>(i - 15)] >> 3);
      const std::uint32_t s1 = RotR32(w[static_cast<std::size_t>(i - 2)], 17) ^
                               RotR32(w[static_cast<std::size_t>(i - 2)], 19) ^
  ...
  digest.resize(32);
  for (int i = 0; i < 8; ++i) {
    const std::uint32_t v = h[static_cast<std::size_t>(i)];
    digest[static_cast<std::size_t>(4 * i + 0)] =
