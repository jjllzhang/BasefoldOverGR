...
struct Profile {
  ...
  std::uint64_t ring_switch_outer_prove_batch_absorb_ns = 0;
  std::uint64_t ring_switch_outer_prove_batch_absorb_calls = 0;

  std::uint64_t ring_switch_outer_prove_batch_prefix_challenge_ns = 0;
  std::uint64_t ring_switch_outer_prove_batch_prefix_challenge_calls = 0;

  std::uint64_t ring_switch_outer_prove_batch_eq_prefix_table_ns = 0;
  std::uint64_t ring_switch_outer_prove_batch_eq_prefix_table_calls = 0;

  std::uint64_t ring_switch_outer_prove_batch_initial_claim_ns = 0;
  std::uint64_t ring_switch_outer_prove_batch_initial_claim_calls = 0;

  std::uint64_t ring_switch_outer_prove_batch_build_g_table_ns = 0;
  std::uint64_t ring_switch_outer_prove_batch_build_g_table_calls = 0;
  ...
  std::uint64_t ring_switch_outer_verify_prefix_absorb_ns = 0;
  std::uint64_t ring_switch_outer_verify_prefix_absorb_calls = 0;

  std::uint64_t ring_switch_outer_verify_prefix_challenge_ns = 0;
  std::uint64_t ring_switch_outer_verify_prefix_challenge_calls = 0;

  std::uint64_t ring_switch_outer_verify_prefix_eq_prefix_table_ns = 0;
  std::uint64_t ring_switch_outer_verify_prefix_eq_prefix_table_calls = 0;

  std::uint64_t ring_switch_outer_verify_prefix_initial_claim_ns = 0;
  std::uint64_t ring_switch_outer_verify_prefix_initial_claim_calls = 0;
  ...
  std::uint64_t ring_switch_outer_verify_final_eq_suffix_table_ns = 0;
  std::uint64_t ring_switch_outer_verify_final_eq_suffix_table_calls = 0;

  std::uint64_t ring_switch_outer_verify_final_g_star_ns = 0;
  std::uint64_t ring_switch_outer_verify_final_g_star_calls = 0;
...
public:
  ...
  ~ScopedTimer() {
    if (ns_acc_ != nullptr)
      *ns_acc_ += (NowNs() - start_ns_);
    if (calls_acc_ != nullptr)
      ++(*calls_acc_);
  }
...
inline void PrintProfile(std::ostream &os, const Profile &p) {
  ...
  if (p.pcs_prove_calls > 0) {
    ...
    const bool has_ext_prover_breakdown =
        (p.ext_sumcheck_init_calls + p.ext_sumcheck_current_poly_calls +
         ...
         p.ext_merkle_tree_open_calls + p.ext_msg0_coeffs_suffix_fold_calls) >
        0;

    const std::uint64_t accounted_ns =
        p.encode_foldable_unchecked_ns + p.sumcheck_init_ns +
        p.sumcheck_current_poly_ns + p.sumcheck_receive_challenge_ns +
        p.prover_commit_round_ns + p.merkle_tree_build_ns +
        p.merkle_tree_open_ns + p.ext_sumcheck_init_ns +
        p.ext_sumcheck_current_poly_ns + p.ext_sumcheck_receive_challenge_ns +
        p.ext_prover_commit_round_ns + p.ext_merkle_tree_build_ns +
        p.ext_merkle_tree_open_ns + p.ext_msg0_coeffs_suffix_fold_ns +
        p.z2k_backend_prove_ns + p.transcript_absorb_ns +
        p.transcript_challenge_ns;
    const double other_ms = (p.pcs_prove_ns > accounted_ns)
                                ? NsToMs(p.pcs_prove_ns - accounted_ns)
                                : 0.0;
    ...
    if (has_ext_prover_breakdown) {
      os << "    ExtensionSumcheck total:     " << ext_sumcheck_total_ms
         << " ms\n";
  ...
  const bool has_ring_switch_outer_prover_breakdown =
      (p.ring_switch_outer_prove_total_calls +
       ...
       p.ring_switch_outer_prove_batch_absorb_calls +
       p.ring_switch_outer_prove_batch_prefix_challenge_calls +
       p.ring_switch_outer_prove_batch_eq_prefix_table_calls +
       p.ring_switch_outer_prove_batch_initial_claim_calls +
       p.ring_switch_outer_prove_batch_build_g_table_calls +
  ...
  if (has_ring_switch_outer_prover_breakdown) {
    ...
    const double batch_absorb_ms =
        NsToMs(p.ring_switch_outer_prove_batch_absorb_ns);
    const double batch_challenge_ms =
        NsToMs(p.ring_switch_outer_prove_batch_prefix_challenge_ns);
    const double batch_eq_prefix_ms =
        NsToMs(p.ring_switch_outer_prove_batch_eq_prefix_table_ns);
    const double batch_initial_claim_ms =
        NsToMs(p.ring_switch_outer_prove_batch_initial_claim_ns);
    ...
    const double total_ms = (p.ring_switch_outer_prove_total_calls > 0)
    ...
    os << "      TranscriptAbsorb:           " << batch_absorb_ms << " ms"
       << "  (calls " << p.ring_switch_outer_prove_batch_absorb_calls << ")\n";
    os << "      PrefixChallengesRPrime:     " << batch_challenge_ms << " ms"
       << "  (calls " << p.ring_switch_outer_prove_batch_prefix_challenge_calls
       << ")\n";
    os << "      BuildEqPrefixTable:         " << batch_eq_prefix_ms << " ms"
       << "  (calls " << p.ring_switch_outer_prove_batch_eq_prefix_table_calls
       << ")\n";
    os << "      InitialClaim:               " << batch_initial_claim_ms
       << " ms"
       << "  (calls " << p.ring_switch_outer_prove_batch_initial_claim_calls
       << ")\n";
    os << "      BuildBatchedGTable:         " << batch_build_g_table_ms
       << " ms"
       << "  (calls " << p.ring_switch_outer_prove_batch_build_g_table_calls
       << ")\n";
  ...
  if (p.pcs_verify_calls > 0) {
    ...
    const double outside_query_ms =
        (p.pcs_verify_ns > total_query_ns + p.z2k_backend_verify_ns)
            ? NsToMs(p.pcs_verify_ns - total_query_ns - p.z2k_backend_verify_ns)
            : 0.0;
    ...
    if (p.z2k_backend_verify_calls > 0) {
      os << "    Z2kBackendVerifyEval:       " << z2k_backend_verify_ms << " ms"
  ...
  const bool has_ring_switch_outer_verifier_breakdown =
      (p.ring_switch_outer_verify_total_calls +
       ...
       p.ring_switch_outer_verify_prefix_challenge_calls +
       p.ring_switch_outer_verify_prefix_eq_prefix_table_calls +
       p.ring_switch_outer_verify_prefix_initial_claim_calls +
       p.ring_switch_outer_verify_sumcheck_replay_calls +
       p.ring_switch_outer_verify_final_check_calls +
       p.ring_switch_outer_verify_final_eq_suffix_table_calls +
  ...
  if (has_ring_switch_outer_verifier_breakdown) {
    ...
    const double prefix_absorb_ms =
        NsToMs(p.ring_switch_outer_verify_prefix_absorb_ns);
    const double prefix_challenge_ms =
        NsToMs(p.ring_switch_outer_verify_prefix_challenge_ns);
    const double prefix_eq_prefix_ms =
        NsToMs(p.ring_switch_outer_verify_prefix_eq_prefix_table_ns);
    const double prefix_initial_claim_ms =
        NsToMs(p.ring_switch_outer_verify_prefix_initial_claim_ns);
    ...
    const double final_eq_suffix_ms =
        NsToMs(p.ring_switch_outer_verify_final_eq_suffix_table_ns);
    const double final_g_star_ms =
        NsToMs(p.ring_switch_outer_verify_final_g_star_ns);
    const double profiled_ms =
        tensor_ms + recover_ms + prefix_ms + sumcheck_ms + final_check_ms;
    const double total_ms = (p.ring_switch_outer_verify_total_calls > 0)
    ...
    const double prefix_other_ms = (prefix_ms > prefix_profiled_ms)
                                       ...
                                       : 0.0;
    const double final_profiled_ms = final_eq_suffix_ms + final_g_star_ms;
    const double final_other_ms = (final_check_ms > final_profiled_ms)
                                      ? (final_check_ms - final_profiled_ms)
                                      : 0.0;
    ...
    os << "    ReplayPrefixBatching:         " << prefix_ms << " ms"
       ...
       << ")\n";
    os << "      TranscriptAbsorb:           " << prefix_absorb_ms << " ms"
       << "  (calls " << p.ring_switch_outer_verify_prefix_absorb_calls
       << ")\n";
    os << "      PrefixChallengesRPrime:     " << prefix_challenge_ms << " ms"
       << "  (calls " << p.ring_switch_outer_verify_prefix_challenge_calls
       << ")\n";
    os << "      BuildEqPrefixTable:         " << prefix_eq_prefix_ms << " ms"
       << "  (calls " << p.ring_switch_outer_verify_prefix_eq_prefix_table_calls
       << ")\n";
    os << "      InitialClaim:               " << prefix_initial_claim_ms
       << " ms"
       << "  (calls " << p.ring_switch_outer_verify_prefix_initial_claim_calls
       << ")\n";
    os << "      Other/prefix-unaccounted:   " << prefix_other_ms << " ms\n";
    ...
    os << "      EvaluateGstar:              " << final_g_star_ms << " ms"
       << "  (calls " << p.ring_switch_outer_verify_final_g_star_calls << ")\n";
    os << "      Other/final-unaccounted:    " << final_other_ms << " ms\n";
    os << "    Other/unaccounted:            " << other_ms << " ms\n";
  }
  ...
  if (has_frobenius_outer_prover_breakdown) {
    ...
    const double batch_prep_ms = NsToMs(p.frobenius_outer_prove_batch_prep_ns);
    const double batch_absorb_ms =
    ...
    const double profiled_ms = orbit_ms + compute_s_ms + recover_ms +
                               batch_prep_ms + sumcheck_ms + final_check_ms;
    const double total_ms = (p.frobenius_outer_prove_total_calls > 0)
    ...
    if (orbit_profiled_ms > 0.0) {
      os << "      RecoverPointCoords:         " << orbit_recover_ms << " ms"
         << "  (calls " << p.frobenius_outer_prove_orbit_recover_coords_calls
         << ")\n";
      ...
      os << "      BaseEqTable:               " << orbit_base_eq_table_ms
         << " ms"
         << "  (calls " << p.frobenius_outer_prove_orbit_base_eq_table_calls
         << ")\n";
      os << "      BaseEqCoords:              " << orbit_base_eq_coords_ms
         << " ms"
         << "  (calls " << p.frobenius_outer_prove_orbit_base_eq_coords_calls
         << ")\n";
      os << "      Other/orbit-unaccounted:    " << orbit_other_ms << " ms\n";
    }
    ...
    if (batch_profiled_ms > 0.0) {
      ...
      os << "      PrefixChallengesLambda:     " << batch_prefix_ms << " ms"
         << "  (calls " << p.frobenius_outer_prove_batch_prefix_challenge_calls
         << ")\n";
      os << "      InitialClaim:               " << batch_initial_claim_ms
         << " ms"
         << "  (calls " << p.frobenius_outer_prove_batch_initial_claim_calls
         << ")\n";
      os << "      BuildBatchedGTable:         " << batch_g_table_ms << " ms"
         << "  (calls " << p.frobenius_outer_prove_batch_build_g_table_calls
         << ")\n";
      os << "      Other/batch-unaccounted:    " << batch_other_ms << " ms\n";
    }
    os << "    SumcheckSuffixRounds:         " << sumcheck_ms << " ms"
  ...
  if (has_frobenius_outer_verifier_breakdown) {
    ...
    const double prefix_ms = NsToMs(p.frobenius_outer_verify_prefix_replay_ns);
    const double sumcheck_ms =
    ...
    const double total_ms = (p.frobenius_outer_verify_total_calls > 0)
