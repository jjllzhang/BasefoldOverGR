...
void TestIOPP_MerkleOpenings() {
  testutil::PrintInfo("IOPP: Merkle instantiation openings verify");
  ...
  msg.SetLength(basefold::MessageLength(params));
  for (long i = 0; i < msg.length(); ++i) {
    msg[i] = ElemFromIndexGF4(static_cast<int>((3 * i + 1) % 4), alpha);
  }
  ...
  basefold::IOPPChallenges challenges;
  challenges.alphas.resize(static_cast<std::size_t>(params.d));
  challenges.alphas[0] = alpha + testutil::ConstZZpE(1);
  challenges.alphas[1] = alpha;

  basefold::IOPPOracles oracles;
  basefold::ProverCommitAll(oracles, pi_d, challenges, params);

  basefold::IOPPMerkleCommitments commitments;
  commitments.roots_by_level.resize(static_cast<std::size_t>(params.d + 1));
  for (long i = 0; i <= params.d; ++i) {
    commitments.roots_by_level[static_cast<std::size_t>(i)] =
        basefold::MerkleCommitOracle(oracles.pi[static_cast<std::size_t>(i)]);
  }

  const long mu0 = 6;
  const basefold::IOPPQueryPlan plan = basefold::MakeQueryPlan(mu0, params);

  basefold::IOPPQueryMerkleOpenings mopen;
  mopen.left.resize(static_cast<std::size_t>(params.d));
  mopen.right.resize(static_cast<std::size_t>(params.d));
  mopen.folded.resize(static_cast<std::size_t>(params.d));
  for (long i = 0; i < params.d; ++i) {
    const long mu = plan.mu_by_level[static_cast<std::size_t>(i)];
    const long ni = basefold::CodewordLengthAtLevel(params, i);
    mopen.left[static_cast<std::size_t>(i)] =
        basefold::MerkleOpenOracle(oracles.pi[static_cast<std::size_t>(i + 1)],
                                   mu);
    mopen.right[static_cast<std::size_t>(i)] =
        basefold::MerkleOpenOracle(oracles.pi[static_cast<std::size_t>(i + 1)],
                                   mu + ni);
    mopen.folded[static_cast<std::size_t>(i)] =
        basefold::MerkleOpenOracle(oracles.pi[static_cast<std::size_t>(i)], mu);
  }
  mopen.pi0_full = oracles.pi[0];

  CHECK(basefold::VerifyQueryFromMerkleOpenings(plan, challenges, mopen,
                                                commitments, params));

  const long n_d = basefold::CodewordLengthAtLevel(params, params.d);
  basefold::MerkleOpening open0 =
      basefold::MerkleOpenOracle(oracles.pi[static_cast<std::size_t>(params.d)],
                                 0);
  CHECK(basefold::MerkleVerifyOpening(
      commitments.roots_by_level[static_cast<std::size_t>(params.d)], n_d,
      open0));

  open0.value += testutil::ConstZZpE(1);
  CHECK(!basefold::MerkleVerifyOpening(
      commitments.roots_by_level[static_cast<std::size_t>(params.d)], n_d,
      open0));
}

void TestIOPP_GR42_MerkleOpenings() {
  testutil::PrintInfo("IOPP: Merkle openings verify over GR(4,2)");
  ...
  vec_ZZ_pE msg;
  msg.SetLength(basefold::MessageLength(params));
  msg[0] = testutil::ConstZZpE(3);
  msg[1] = alpha + testutil::ConstZZpE(1);
  msg[2] = testutil::ConstZZpE(2);
  msg[3] = alpha;
