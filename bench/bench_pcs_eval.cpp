...
SizeStats ComputeSizeStats(const std::vector<std::uint64_t> &xs) {
  SizeStats out;
  if (xs.empty())
    return out;
  ...
  for (std::uint64_t v : xs) {
    ...
    out.min_bytes = std::min(out.min_bytes, v);
    out.max_bytes = std::max(out.max_bytes, v);
  }
  out.mean_bytes =
      static_cast<double>(sum / static_cast<long double>(xs.size()));
  return out;
}
...
std::size_t SerializedFieldElementSize(const ZZ_pE &x) {
  const long r = ZZ_pE::degree();
  if (r <= 0) LogicError("SerializedFieldElementSize: invalid extension degree");
  ...
  for (long i = 0; i < r; ++i) {
    ...
    if (n < 0) LogicError("SerializedFieldElementSize: NumBytes returned < 0");
    total += 8 + static_cast<std::size_t>(n);
  }
  return total;
}

std::size_t SerializedMerkleOpeningSize(const basefold::MerkleOpening &o) {
  std::size_t total = 0;
  ...
  if (o.auth_path.sibling_hashes.size() != o.auth_path.sibling_is_left.size()) {
    LogicError("SerializedMerkleOpeningSize: auth_path size mismatch");
  }
  for (const basefold::Bytes &h : o.auth_path.sibling_hashes) {
    total += h.size();  // hashes are fixed-size (sha256), so no length prefix
  }
  total += o.auth_path.sibling_is_left.size();  // 1 byte per direction bit
  return total;
}

std::uint64_t EstimateEvalProofSizeBytes(const basefold::BaseFoldPCSEvalProof &p) {
  std::uint64_t total = 0;

  // Merkle roots (π_0..π_d).
  for (const basefold::MerkleRoot &r : p.commitments.roots_by_level) {
    total += static_cast<std::uint64_t>(r.size());
  }

  // Sumcheck messages h_i (quadratic polys).
  for (const basefold::QuadraticPoly &h : p.h_by_level) {
  ...
  // Query openings.
  for (const basefold::BaseFoldPCSQueryProof &qp : p.query_proofs) {
    for (const basefold::MerkleOpening &o : qp.left) {
      total += static_cast<std::uint64_t>(SerializedMerkleOpeningSize(o));
    }
    for (const basefold::MerkleOpening &o : qp.right) {
      total += static_cast<std::uint64_t>(SerializedMerkleOpeningSize(o));
    }
    for (const basefold::MerkleOpening &o : qp.folded) {
      total += static_cast<std::uint64_t>(SerializedMerkleOpeningSize(o));
    }
  }
...
BenchResult RunEvalBenchmark(const vec_ZZ_pE &f_coeffs,
  ...
  for (int iter = -warmup; iter < reps; ++iter) {
    ...
    const std::uint64_t sz = EstimateEvalProofSizeBytes(proof);

    // Prevent over-optimization: fold in a few bytes + sizes.
    if (!C.empty())
  ...
  out.proof_size = ComputeSizeStats(proof_bytes);
  out.sink = sink;
  return out;
}
...
void PrintResult(const std::string &label, long mod, long c, long d,
  ...
  std::cout << "  proof size mean " << static_cast<std::uint64_t>(r.proof_size.mean_bytes)
            << " B  (min " << r.proof_size.min_bytes << ", max "
            << r.proof_size.max_bytes << ")\n";
