...
BenchResult RunVerifyBenchmark(
  ...
  const double proof_size_kb = static_cast<double>(proof_size_bytes) / 1024.0;
  ...
  out.proof_size_kb = proof_size_kb;
...
void PrintResult(const std::string &label, const ZZ &scalar_modulus, long c,
  ...
  std::cout << "  input proof size " << r.proof_size_kb << " KB  ("
...
void PrintHelp() {
  std::cout
      ...
      << "Notes:\n"
      << "  Commitment and proof generation happen before timed verify.\n"
      << "  Exact fixed-width proof payload size is reported for the verifier input proof.\n";
}
...
void RunOneContext(const ContextSpec &spec, long c, long k0, long d,
  ...
  const BenchResult r = RunVerifyBenchmark(
