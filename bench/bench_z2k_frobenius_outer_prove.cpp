...
BenchResult RunProveBenchmark(const basefold::FrobeniusPCSParams &params,
  ...
  if (reps <= 0) {
    NTL::LogicError("RunProveBenchmark: reps must be > 0");
  }
  if (num_queries < 0) {
    NTL::LogicError("RunProveBenchmark: num_queries must be >= 0");
  }
  ...
  for (int iter = -warmup; iter < reps; ++iter) {
    ...
    if (iter >= 0) {
      prove_ms.push_back(MsSince(t0, t1));
    }
  }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const auto setup = BuildFrobeniusBenchSetupResult(
        FrobeniusBenchCalibrationMode::kOuterProve, c, ell, kappa, spec, F,
        zeta);
